    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between">
          <span className="font-label font-semibold text-xl text-text-primary">
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-border-light bg-white sticky top-0 z-10">
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="font-label font-semibold text-gold-deep">
                          ${item.price.toFixed(2)}
                        </p>
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          ...
                          <span className="font-label font-medium text-sm w-6 text-center">
                            {item.quantity}
                          </span>
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="p-6 border-t border-border-light bg-white sticky bottom-0">
                ...
                <button className="w-full relative flex items-center justify-center btn-primary text-white py-3 rounded-xl font-medium transition-all duration-300 hover:scale-[1.02]">
                  <ChevronRight className="w-4 h-4 absolute left-5" />
                  <span>Checkout</span>
                </button>

                <button
                  onClick={clearCart}
                  className="w-full mt-2 py-2 text-sm text-text-muted hover:text-crimson font-medium transition-colors"
                >
                  Clear Cart
                </button>
              </div>
...
export default function Menu() {
  ...
  return (
    <>
      <Helmet>
        <title>Our Menu | The Pearl Pakistan Restaurant</title>

        <meta
          name="description"
          content="Explore our full menu of authentic Pakistani dishes."
        />
      </Helmet>
      ...
      <main className=" bg-cream">
        ...
        <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden bg-charcoal">
          ...
          <div className="relative z-10 text-center">
            ...
            >
              ...
              <h1 className="font-display text-5xl md:text-6xl lg:text-7xl text-ivory">
                Our Menu
              </h1>
            </motion.div>
          </div>
        ...
        {/* FILTER BAR FIXED */}
        <section className="relative z-10 bg-cream border-b border-border-light py-4">
          <div className="content-max-width px-4 md:px-6">
            <div className="flex gap-3 overflow-x-auto scrollbar-hide">
              {menuCategories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveFilter(cat)}
                  className={`relative px-5 py-2.5 rounded-full font-medium text-sm whitespace-nowrap transition-all duration-300 ${
                    activeFilter === cat
                      ? 'text-white'
        ...
        {/* MENU GRID */}
        <section className="section-padding bg-cream pt-14">
          <div className="content-max-width px-4 md:px-6">
            <motion.div
              layout
